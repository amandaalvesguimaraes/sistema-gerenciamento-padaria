...
export class VencimentoProdutosComponent implements OnInit {
  ...
  searchProduto() {
    ...
    this.validadeT = this.datepipe.transform(this.validade, 'dd/MM/yyyy');
    for (let i : number = 0; i < this.produtos.length; i++) {
      let val = this.datepipe.transform(this.produtos[i].validade, 'dd/MM/yyyy');
      if (val == this.validadeT) {
        console.log(val);
        console.log(this.validadeT);
